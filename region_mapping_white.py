...
ref_table = np.loadtxt(rl + '_ref_table.txt')
os.chdir(os.path.join(PRD, 'surface'))
vl = np.loadtxt(rl + '_white_vertices_low.txt')  # vertices low
vh = np.loadtxt(rl + '_white_vertices_high.txt')  # vertices high
reg_map = []
for vli in vl:
    pos = np.argmin(np.sum(np.abs(vh - vli), 1))
    find_tab = np.nonzero(ref_table[:, 5] == L[pos])[0][0]
    reg_map.append(ref_table[find_tab, 4])
np.savetxt(rl + '_white_region_mapping_low_not_corrected.txt', reg_map)
